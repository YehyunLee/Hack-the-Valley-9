...
export default function Trashcam(props: any) {
  let bubbleContainer: HTMLElement | null;
  let sessionData = props.sessionData;
  const [trashcamPrompt, setTrashcamPrompt] = useState('');
    ...
    const getTrashcamPrompt = async () => {
      try {
        if (sessionData?.user?.id) {
          ...
          setTrashcamPrompt(response.data.Topic);
        }
      } catch (error) {
  ...
  return (
    <>
      <p className="pl-8 pb-8 pt-8 mr-[1rem] ml-[1rem] text-lg text-gray-700 font-semibold rounded-3xl bg-DAF2F1 shadow-2xl w-auto"> {trashcamPrompt} </p>
      < div id="bubbleContainer" className="w-[100vw] flex-col ">
        {/* <div className="bubble right">Ok, Thank you</div> */}
      </div>
      <VoiceToText sendTranscriptToParent={showTranscript} sessionData={sessionData} trashcamPrompt={trashcamPrompt} sendTranscriptToBot={showAnswer} />
